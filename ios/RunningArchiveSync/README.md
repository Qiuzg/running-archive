...
心率、跑步功率与步数按 Workout 的精确起止时间查询，而不是只查询 HealthKit 中
显式关联到 Workout 的样本。后者在部分旧 Apple Watch 记录中只能返回 2–9 个心率
点，会让网页曲线近似一条直线。安装包含该修复的版本后，App 会把 2026-04-25 及
以前曾同步过的记录重新列为待同步；再次上传会覆盖对应记录并修复曲线，不会产生
重复跑步。
